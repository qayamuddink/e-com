import { useEffect, useState } from "react"


interface Author {
    name:string;
    isFollowing:boolean;
    image:string;
}

const TopSellar = () => {
    
    const [authors , setAuthor] = useState<Author[]>([])

    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await fetch(`https://randomuser.me/api/?results=10`);
                const data = await response.json()

                const authorsData :Author[] = data.results.map((user:any) => ({
                    name:`${user.name.first} ${user.name.last}`,
                    isFollowing:false,
                    image:user.picture.medium,                    
                }))
                console.log(authorsData);
                
                setAuthor(authorsData)
            } catch (error) {
                console.error(`Error while Authors data ${error}`);
                
            }
        }
        fetchData()
    } ,[])

    const handleFollowClick =(index:number) => {
        setAuthor(prevAuthor => 
            prevAuthor.map((author , i) => 
                i === index ? {...author , isFollowing: !author.isFollowing } : author))
    }


    return <div className="bg-white p-5 mx-5 mt-5 border w-[23rem] rounded">
         <h2 className="text-xl font-bold mb-5">Top Sellers</h2>
         <ul>
            {authors.map((author , index) => (
                <li key={index} className="flex items-center justify-between mb-4"> 
                    <section className="flex justify-center items-center">
                        <img src={author.image} alt={author.name} className="w-[25%] h-[25%] rounded-full " />
                        <span className="ml-4">{author.name}</span>
                    </section>

                    <button  onClick={() => handleFollowClick(index)}
                     className={`py-1 px-3 cursor-pointer ${author.isFollowing ? "bg-red-300 text-white rounded-md" : "bg-black text-white rounded-md"} `}>
                        {author.isFollowing ?"Unfollow" : "Follow"}
                    </button>
                 </li>
            ))}
         </ul>
         </div>
}

export default TopSellar